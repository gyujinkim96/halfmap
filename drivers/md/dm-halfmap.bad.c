...
#define SSD_CAPACITY (32 * 1024 * 1024ULL)
...
static void initialize_block_management(void) {
    ...
    printk(KERN_ERR "halfmap: initialize_block_management\n");
...
static void halfmap_end_io(struct bio *bio)
{
    ...
    printk(KERN_ERR "halfmap: halfmap_end_io\n");
...
static int halfmap_map(struct dm_target *ti, struct bio *bio)
{
    ...
    if (bio_data_dir(bio) == WRITE) {
        write = true;
        printk(KERN_ERR "halfmap write cmd\n");
    } else {
        printk(KERN_ERR "halfmap read cmd\n");
    }
    ...
    sector_t lba = bio->bi_iter.bi_sector;  // starting sector of the bio

    printk(KERN_ERR "halfmap_map");
    // printk(KERN_ERR "start bio sector: %d", bio_sectors(bio));
    // for (i = 0; i < bio_sectors(bio) - SECTORS_PER_PAGE; i += SECTORS_PER_PAGE) {  // 4K = 8 sectors
    //     uint32_t curr_phy_blk_map;
    ...
    //     printk(KERN_ERR "last bio sector: %d", sector);
    //     curr_phy_blk_map = physical_block_map[sector / SECTORS_PER_PAGE];
    //     printk(KERN_ERR "   blk map: %d", curr_phy_blk_map);
    ...
    bio_set_dev(current_bio, target_dev->bdev);
    submit_bio(current_bio);
    printk(KERN_ERR "halfmap map done2");
